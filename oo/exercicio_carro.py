...
class Motor():
    ...
    def frear(self):
        self.velocidade -= 2 if self.velocidade > 1 else 1 if self.velocidade > 0 else 0


class Direcao():
    VALORES_DIRECAO = ('Norte', 'Leste', 'Sul', 'Oeste')

    def __init__(self, valor='Norte'):
        self.valor = valor

    def girar_a_direita(self):
        posicao_atual = self.VALORES_DIRECAO.index(self.valor)
        nova_posicao = posicao_atual + 1 if posicao_atual < 3 else 0
        self.valor = self.VALORES_DIRECAO[nova_posicao]

    def girar_a_esquerda(self):
        posicao_atual = self.VALORES_DIRECAO.index(self.valor)
        nova_posicao = posicao_atual - 1 if posicao_atual > 0 else 3
        self.valor = self.VALORES_DIRECAO[nova_posicao]
